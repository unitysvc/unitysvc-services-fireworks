// data/fireworks/services/deepseek-r1-0528/deepseek-r1-0528_listing_code_example.js
...
var uuid4 = function () {
  ...
  const randomByte = crypto2
    ? () => crypto2.getRandomValues(u8)[0]
    : () => (Math.random() * 255) & 255;
...
var defaults = {
  ...
  serializeDate(date) {
    return (
      toISOString ??
      (toISOString = Function.prototype.call.bind(Date.prototype.toISOString))
    )(date);
  },
  skipNulls: false,
...
var EventStream = class {
  ...
  on(event, listener) {
    const listeners =
      __classPrivateFieldGet(this, _EventStream_listeners, "f")[event] ||
      (__classPrivateFieldGet(this, _EventStream_listeners, "f")[event] = []);
   ...
   */
  off(event, listener) {
    const listeners = __classPrivateFieldGet(this, _EventStream_listeners, "f")[
      event
    ];
  ...
  _emit(event, ...args) {
    ...
    if (event === "end") {
      ...
      __classPrivateFieldGet(this, _EventStream_resolveEndPromise, "f").call(
        this
      );
    }
    const listeners = __classPrivateFieldGet(this, _EventStream_listeners, "f")[
    ...
    if (event === "abort") {
      ...
      __classPrivateFieldGet(this, _EventStream_rejectEndPromise, "f").call(
        this,
        error
      );
      this._emit("end");
      ...
      for (const tool_call of message.tool_calls) {
        ...
        if (!fn) {
          const content2 = `Invalid tool_call: ${JSON.stringify(
            name
          )}. Available options are: ${Object.keys(functionsByName)
    ...
    function _AbstractChatCompletionRunner_getFinalFunctionToolCall2() {
      for (let i = this.messages.length - 1; i >= 0; i--) {
        ...
        if (isAssistantMessage(message) && message?.tool_calls?.length) {
          return message.tool_calls.filter((x) => x.type === "function").at(-1)
            ?.function;
        }
      }
      return;
...
var brand_privateNullableHeaders = /* @__PURE__ */ Symbol(
...
var AssistantStream = class extends EventStream {
  ...
  async finalRunSteps() {
    ...
    return Object.values(
      __classPrivateFieldGet(this, _AssistantStream_runStepSnapshots, "f")
    );
  }
  async finalMessages() {
    await this.done();
    ...
    const headers = buildHeaders([
      options?.headers,
      {
        ...
        "X-Stainless-Custom-Poll-Interval":
          options?.pollIntervalMs?.toString() ?? void 0,
      },
    ]);
    while (true) {
...
var ResponseStream = class _ResponseStream extends EventStream {
  ...
  async _createOrRetrieveResponse(client, params, options) {
    ...
    for await (const event of stream) {
      ...
      ).call(this, event, starting_after);
    }
    if (stream.controller.signal?.aborted) {
      throw new APIUserAbortError();
    }
  ...
  (_ResponseStream_addEvent = function _ResponseStream_addEvent2(
    event,
    starting_after
  ) {
    if (this.ended) return;
  ...
  (_ResponseStream_accumulateResponse =
    function _ResponseStream_accumulateResponse2(event) {
      let snapshot = __classPrivateFieldGet(
        this,
        _ResponseStream_currentResponseSnapshot,
        "f"
...
(_Webhooks_instances = /* @__PURE__ */ new WeakSet()),
  ...
  (_Webhooks_getRequiredHeader = function _Webhooks_getRequiredHeader2(
    headers,
    name
  ) {
    if (!headers) {
...
var OpenAI = class {
  ...
  async makeRequest(optionsInput, retriesRemaining, retryOfRequestLogID) {
    ...
    const responseInfo = `[${requestLogID}${retryLogStr}${specialHeaders}] ${
      req.method
    } ${url} ${response.ok ? "succeeded" : "failed"} with status ${
      response.status
    } in ${headersTime - startTime}ms`;
    if (!response.ok) {
  ...
  buildBody({ options: { body, headers: rawHeaders } }) {
    ...
    if (
      ...
      return __classPrivateFieldGet(this, _OpenAI_encoder, "f").call(this, {
        body,
        headers,
      });
    }
